...
{
	TileType::tileTypes = {
	{'0',new TileType('0', "Air", texturePath + "BlockSky.png", true)},
	{'1',new TileType('1', "Ground", texturePath + "BlockPlatform.png", false)},
	{'2',new TileType('2', "Lava", texturePath + "LavaAnimated.gif", false)}/*,
	{'c',new TileType('c', "Coin", texturePath + "CoinAnimated.gif")},
	{'e',new TileType('e', "Enemy Spawn", texturePath + "EnemyAlive.png")},
	{'s',new TileType('s', "Player Spawn", texturePath + "Door.png")},
...
void LoadTileTextures(int tileWidth, int tileHeight) {
	...
	for (auto itr = GameObjectType::gameObjectTypes.begin(); itr != GameObjectType::gameObjectTypes.end(); ++itr) {
		sf::Texture * texture = new sf::Texture();
		if (!texture->loadFromFile(itr->second->spriteLoc, sf::IntRect(0, 0, tileWidth, tileHeight)))
		{
		}
		else {
			GameObjectType::gameObjectTypeTextures.insert({ itr->first,texture });
		}
	}
}
...
void LevelEditorLoadLevel(Level * pLevel) {
	...
	for (int y = 0; y < pLevel->gridSize.y; y++) {
		for (int x = 0; x < pLevel->gridSize.x; x++) {
			if (pLevel->GetTileAtCoord(x, y) != nullptr) {
				views[ViewName::LevelEditor]->AddSprite(pLevel->GetTileAtCoord(x, y)->GetSprite());
				views[ViewName::LevelEditor]->AddSprite(pLevel->GetTileAtCoord(x, y)->GetGameObjectSprite());
			}
		}
	}
...
TileType* activeTileBrush;
GameObjectType* activeGameObjectBrush;
...
void InitLevelEditorSideBarView() {
	...
	// Txt GameObject Brushes
	sf::Text * txtGameObjectBrushes = new sf::Text("Game Object:", generalFont, 18);
	sf::Vector2f txtGameObjectBrushesPos(sfView->getSize().x / 2 - txtGameObjectBrushes->getGlobalBounds().width / 2, yOffset);
	txtGameObjectBrushes->setPosition(txtGameObjectBrushesPos);
	yOffset += txtGameObjectBrushes->getGlobalBounds().height + 10;
	editorBarView->AddText(txtGameObjectBrushes);

	xOffset = 10;
	float gameObjectButtonSize = (sfView->getSize().x - 30) / 3;
	for (auto const& entry : GameObjectType::gameObjectTypes) {
		Button * ttb = new Button();
		GameObjectType * got = entry.second;

		sf::RectangleShape * rect = new sf::RectangleShape(sf::Vector2f(gameObjectButtonSize, gameObjectButtonSize));
		ttb->SetShape(rect);
		ttb->SetOutline(2, sf::Color::Color(99, 99, 99, 255));
		ttb->SetPosition(xOffset, yOffset);


		sf::Sprite * btnSprite = new sf::Sprite(*GameObjectType::gameObjectTypeTextures[entry.first], sf::IntRect(0, 0, activeLevel->tileSize, activeLevel->tileSize));
		btnSprite->setScale(gameObjectButtonSize / activeLevel->tileSize, gameObjectButtonSize / activeLevel->tileSize);
		btnSprite->setPosition(xOffset, yOffset);
		ttb->SetSprite(btnSprite);
		...
		ttb->AddMouseEnterFunc(FnMOver); // Assign func to input
		//// Create mouse exit lambda func which changes the BG col
		auto FnMExit = [ttb, got]() {
			if (activeGameObjectBrush != got)
				ttb->SetOutline(2, sf::Color::Color(99, 99, 99, 255));
		};
		ttb->AddMouseExitFunc(FnMExit); // Assign func to input
		//// Create mouse click func
		auto FnMReleased = [ttb, got]() {
			activeTileBrush = nullptr;
			activeGameObjectBrush = got;
		...
		xOffset += gameObjectButtonSize + 5;
		if (xOffset + gameObjectButtonSize >= sfView->getSize().x) {
			xOffset = 10;
			yOffset += gameObjectButtonSize + 5;
		}
	...
	for (int y = 0; y < activeLevel->gridSize.y; y++) {
		for (int x = 0; x < activeLevel->gridSize.x; x++) {
			...
			auto mouseEnterFunction = [x, y]() {
				if (activeLevel->GetTileAtCoord(x, y)->GetTileType() == nullptr || activeTileBrush == nullptr || activeLevel->GetTileAtCoord(x, y)->GetTileType() == activeTileBrush || !leftMouseButtonPressed) {
					return;
				}
				activeLevel->GetTileAtCoord(x, y)->SetTileType(activeTileBrush);
			};
